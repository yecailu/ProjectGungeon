...
{
    public class EnemyDBig : MonoBehaviour, IEnemy
    {
        ...
        public void Hurt(float damage, Vector2 hitDirection)
        {
            ...
            if (HP <= 0)
            {
                FxFactory.PlayEnemyDieBody(transform.Position2D(), hitDirection, "EnemyDDie", 3f);
            ...
            State.State(States.Shoot)
                .OnEnter(() =>
                {
                    ...
                    if (State.SecondsOfCurrentState <= Time.deltaTime * 1.5f)
                    {
                        ...
                        if (Global.Player)
                        {
                            BulletHelper.ShootAround(18, transform.position, 0.5f, EnemyBullet, 8);
