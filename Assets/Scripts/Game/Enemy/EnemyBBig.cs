...
{
    public class EnemyBBig : MonoBehaviour, IEnemy
    {
        ...
        public void Hurt(float damage, Vector2 hitDirection)
        {
            ...
            if (HP <= 0)
            {
                FxFactory.PlayEnemyDieBody(transform.Position2D(), hitDirection, "EnemyBDie", 3f);
                ...
                .OnEnter(() =>
                {
                    ...
                    if (State.SecondsOfCurrentState <= Time.deltaTime * 1.5f)
                    {
                        ...
                        if (Global.Player)
                        {
                            ...
                            BulletHelper.ShootSpread(9, 15, transform.position, directionToPlayer, 0.5f, EnemyBullet, 8);
